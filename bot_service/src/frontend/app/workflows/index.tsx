...
const WorkflowSkeleton = React.memo(() => {
...
const WorkflowsHeader = React.memo(({ 
  setShowCreateDialog, 
  workflowCount, 
  activeCount 
}: { 
    ...
    <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8 content-fade-in">
      <div className="mb-6 lg:mb-0">
        ...
        <p className={`text-lg max-w-2xl ${darkMode ? "text-gray-400" : "text-gray-600"}`}>
          Design and manage multi-agent conversation flows with a visual workflow builder
        </p>
      </div>
      <div className="flex flex-col sm:flex-row gap-3">
        <button
          className={`px-6 py-3 border-2 rounded-xl font-medium transition-all duration-200 group hover-lift ${
            darkMode
...
// Interface for workflow with process status
...
const WorkflowsPage: NextPage = () => {
  ...
  const [workflowToDelete, setWorkflowToDelete] = useState<WorkflowWithStatus | null>(null);
  const [showCreateDialog, setShowCreateDialog] = useState(false);
  const [searchTerm, setSearchTerm] = useState('');
  const [viewMode, setViewMode] = useState<'grid' | 'list'>('grid');
  
  // Memoized filtered workflows for performance
  ...
  const activeCount = useMemo(() => {
    return workflows.filter(wf => 
      wf.processStatus?.status === 'running' && wf.processStatus?.is_workflow_subprocess
    ).length;
  }, [workflows]);
  ...
  const fetchWorkflows = useCallback(async () => {
    ...
    try {
      ...
      // After fetching workflows, check status for each workflow
  ...
  // Function to fetch process status for all workflows
  const fetchProcessStatuses = async (workflowsData: Workflow[]) => {
    const updatedWorkflows = [...workflowsData] as WorkflowWithStatus[];
    
    for (let i = 0; i < updatedWorkflows.length; i++) {
      try {
        // Return a default status object instead of calling the endpoint
        updatedWorkflows[i] = { 
          ...updatedWorkflows[i], 
          processStatus: { status: 'unknown', message: 'Status checking disabled' }
        };
      } catch (err) {
        console.error(`Error updating status for workflow ${updatedWorkflows[i].id}:`, err);
      }
    }
  ...
  // Function to terminate a workflow process
  const handleTerminateProcess = async (workflowId: string, e: React.MouseEvent) => {
    e.preventDefault(); // Prevent navigation
    e.stopPropagation(); // Stop event propagation
    
    // Show loading toast
    const loadingToastId = toast.loading('Terminating process...', { 
      style: {
    ...
    try {
      // First attempt to terminate
      await workflowService.terminateWorkflowProcess(workflowId);
      
      // Wait for process to terminate (maximum 5 seconds)
      let status = { status: 'terminated', message: 'Process terminated' };
      let attempts = 0;
      const maxAttempts = 5;
      
      // Wait for a short delay to allow the termination to complete
      await new Promise(resolve => setTimeout(resolve, 3000));
      
      // Update UI with final status
      setWorkflows(prevWorkflows => 
        prevWorkflows.map(wf => 
          wf.id === workflowId ? { ...wf, processStatus: status } : wf
        )
      );
      ...
      toast.dismiss(loadingToastId);
      toast.error('Failed to terminate process. Please try again or restart the service.', {
        style: {
          background: darkMode ? '#1E1E1E' : '#FEF2F2',
          color: darkMode ? '#F87171' : '#DC2626',
          borderColor: darkMode ? '#991B1B' : '#FECACA',
  ...
  const handleDeleteClick = (workflow: WorkflowWithStatus, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  useEffect(() => {
    ...
    // Set up polling for statuses every 5 seconds
    const interval = setInterval(() => {
      if (workflows.length > 0) {
        fetchProcessStatuses(workflows);
      }
    }, 5000);
  ...
  // Clean up polling interval when workflows change
  useEffect(() => {
    return () => {
      if (statusPollingInterval) {
        clearInterval(statusPollingInterval);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            searchTerm ? (
              <div className={`rounded-2xl border shadow-xl p-12 text-center content-fade-in glass-effect ${
                ...
                <p className={`text-lg mb-8 max-w-md mx-auto ${
                  ...
                  No workflows match your search for "{searchTerm}". Try adjusting your search terms.
                </p>
                <button
            ...
            darkMode 
                ...
                <div className={`inline-flex items-center justify-center w-24 h-24 rounded-full mb-8 ${
                  ...
                  <GitBranch size={48} className={darkMode ? "text-blue-400" : "text-blue-600"} />
            </div>
              ...
              Create Your First Workflow
                ...
                <p className={`text-lg mb-8 max-w-2xl mx-auto ${
                  ...
                  You haven't created any workflows yet. Build multi-agent conversations with our intuitive visual workflow builder and unlock the power of collaborative AI.
            </p>
              ...
              <button
               onClick={() => setShowCreateDialog(true)}
                  ...
                  <button className={`px-6 py-4 border-2 rounded-xl font-medium transition-all duration-200 hover-lift ${
                darkMode 
                      ? "border-gray-600 text-gray-300 hover:bg-gray-700/50 hover:border-gray-500"
                      : "border-gray-300 text-gray-700 hover:bg-gray-50 hover:border-gray-400"
                  } shadow-sm hover:shadow-md gpu-accelerated`}>
            ...
            <div className={`${
              ...
              <WorkflowCard
                ...
                onTerminate={handleTerminateProcess}
              />
                </div>
            ))}
          </div>
